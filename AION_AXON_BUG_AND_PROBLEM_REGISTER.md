...
**COMMIT:** `0c1c2ba` — "Fix BUG-014: catch a mission's wrong argument count before it runs". Not yet pushed or deployed — see the current `AION_AXON_NEXT_SESSION_PROMPT.md` for the exact commands to run from the owner's own machine (push needs a hardcoded Windows Python path in the pre-push hook; deploy needs a browser-based Firebase login neither of which exist on this device-bridge shell).
**REMAINING WORK:** None on this bug. Once the owner pushes and deploys this commit, it's worth a quick live re-check that a deliberately under-supplied mission node now ends the mission `BLOCKED`/`ERROR` with the new clear sentence instead of a traceback — optional, not required, since the local test suite already proves the behavior.
