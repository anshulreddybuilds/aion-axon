...
**SEVERITY:** P1 — FIXED, 30 Aug 2026 (not yet deployed; backend change, needs `gcloud run deploy`)
**AREA:** Backend — natural-language "Plan it" missions could name an existing capability with no arguments and never complete, even with BUG-014's fix in place.
**FILE(S):** `app/missions/engine.py` (`MissionEngine.run()` backfill, new `_minimum_args_required()` static method), `tests/test_mission_engine.py` (5 new tests).
**PROBLEM:** The owner ran a real mission via "Plan it" and hit the exact same capability (`generate_nepal_crisis_image`) failing again — but this time with BUG-014's own clean message (`'generate_nepal_crisis_image' needs 1 argument but this call supplied 0.`), not a crash. The planner (an LLM constrained by `app/agents/plan_schema.py`, where `MissionStep.args` defaults to `[]`) had named the capability correctly but left `args: []`. Root cause: `mission_planner.INSTRUCTION` only documents argument shape for `calculator` and `web_research` by name, and `declarations._signature_of()` deliberately returns `""` for any SYNAPSE-acquired capability, since its registered function is always `_sandbox_proxy`'s bare `invoke(*args)` — the planner was never told this capability takes an argument at all, so it could not have written one.
**HOW DISCOVERED:** Reported live by the owner re-running a similar mission through "Plan it" after BUG-014 shipped, expecting it to now succeed rather than merely fail cleanly.
**IMPACT:** Any "Plan it" mission routed to an existing capability with required arguments could complete the planning step but never actually finish the mission, cleanly rejected every time — correct per BUG-014, but still a dead end for the person running it, with no automatic recovery.
**FIX:** Committed `70d4973`. `app/missions/service.py`'s `resume_blocked()` already had the fix for this *exact* failure mode (see its own "Found live 21 Aug" docstring) but only for the capability-gap-resume path (a step whose `tool` was `null`, now resumed after installing a capability for it). This applies the same backfill — the mission's own free-text request becomes the step's sole argument, only when `args` is still empty, never overwriting an explicit one — to the normal first-run path in `MissionEngine.run()`, gated by a new `_minimum_args_required(tool_name)` so a capability *known* to take zero arguments is never force-fed one (that would just trade BUG-018 for a new, self-inflicted arity error). For a SYNAPSE-acquired capability, the real arity is read from its stored generated source (`passport.candidate.code`/`.entrypoint`, read via `SynapseEngine._entrypoint_arity`) — the exact same record and method `app/capabilities/rehydrate.py` already reads for the same underlying reason (the registered function's own signature reveals nothing). For a hand-written seed capability, `inspect.signature` on the real function is enough. An indeterminate minimum (can't be read, or a genuine `*args`) is treated as "needs at least one," matching `resume_blocked()`'s own unconditional behavior.
**REGRESSION TEST:** 5 new tests in `tests/test_mission_engine.py`: empty args on an existing tool get backfilled with the mission's request; explicit args are never overwritten; a confirmed zero-arg capability is not force-fed an argument; the backfill correctly reads arity from a stored SYNAPSE candidate rather than the bare sandbox-proxy signature (the real-world shape); an indeterminate (genuinely variadic) minimum still backfills rather than being mistaken for zero.
**VERIFICATION:** Full suite independently reproduced twice (once by the implementing pass, once as a review check before committing): **620 passed, 4 skipped** — exactly the pre-existing 615/4 baseline plus these 5 new tests, no unrelated shift. The diff was also read line-by-line before committing and cross-checked against `rehydrate.py`'s existing, working use of the identical `passport.candidate` field path, rather than trusting the report alone.
**COMMIT:** `70d4973` — "fix(BUG-018): backfill a mission step's empty args on first run, not just on resume".
**REMAINING WORK:** Needs a **backend** deploy this time (unlike BUG-015/016/017): `gcloud run deploy aion-core --source . --region asia-south1 --project aion-axon-2026`, in addition to the usual `git push`. Known, deliberately-unfixed residual gap: `resume_planned()` executes the first resumed step directly via `orchestrator.approve_and_resume()` before ever reaching `run()` for the rest of the plan, so a step that both needs approval AND hits this exact empty-args-on-an-existing-tool condition, specifically on that one first resumed step, bypasses the backfill. Narrow (needs MEDIUM/HIGH risk stacked with empty args and an existing tool, on that specific step) and documented here rather than chased given the current time constraint — worth a follow-up after the deadline.
...
**SEVERITY:** P2 — FIXED, 29 Aug 2026 (not yet deployed)
**AREA:** Frontend — stale hardcoded model-name labels displayed as the live engine.
**FILE(S):** `web/src/v2/CommandCapsule.jsx` (`MODELS` array), `web/src/v3/AppV3.jsx` (two literal strings), `web/src/v4/AppV4.jsx` (two literal strings). `web/src/demoRecoveryFixture.js` deliberately left unchanged (see below).
**PROBLEM:** Three of the five UI surfaces hardcoded engine names that are no longer true: `"gemini-3.6-flash"` and, in `CommandCapsule.jsx`'s model picker, `"gemma-4-26b-a4b-it"` as the evaluator. Neither model is what actually runs. `app/synapse/generator.py` and `app/synapse/evaluator.py` both default `MODEL` to `"gemini-3.5-flash"` — the Gemma evaluator model (`gemma-3-27b-it`, then its replacement `gemma-4-26b-a4b-it`) 404'd in production on 2026-08-29 and was retired, per `evaluator.py`'s own history comment. `web/src/v5/AppV5.jsx`, `main.jsx`, `Shell.jsx`, `App.jsx`, and `Command.jsx` (the default route and v1's Command view) were confirmed clean via grep — this only affected v2, v3, and v4.
**HOW DISCOVERED:** Found during the same permission-driven, full-check testing pass as BUG-015 (below) — a `grep -rn "gemini-3\.\|gemma-"` sweep across `web/src/` cross-referenced against the real backend `MODEL` defaults, not reported by the owner.
**IMPACT:** Cosmetic/trust, not functional — nothing in these files actually sends `model` anywhere (confirmed for `CommandCapsule.jsx`: the picker's selected value was never read outside its own display), so no request was ever misrouted. But a judge or the owner reading "gemma-4-26b-a4b-it — evaluator" while every real evaluation actually ran on gemini would reasonably conclude the system is lying about its own architecture, at exactly the kind of scrutiny moment (hackathon judging) this matters most.
**FIX:** Committed `31f4d2e`. Both stale strings replaced with `gemini-3.5-flash` everywhere they claimed to be a live/current status. `CommandCapsule.jsx`'s `MODELS` array previously implied two distinct real engines; since both stages now genuinely run the same model, its two entries keep the same `id` but gained a separate `key` field (`"generator"` / `"evaluator"`) so React's `key` prop and the picker's selected-row highlighting don't collide on a now-duplicate `id` — component state (`model` → `modelKey`) and all three read sites were updated to match. `web/src/demoRecoveryFixture.js`'s `DEMO_RECORD.evaluation.model: "gemma-4-26b-a4b-it"` was deliberately left as-is: that file is explicit, labeled mock data for a deterministic offline demo walkthrough (`DEMO_FIXTURE_LABEL` states this on screen), not a live status claim, so "wrong" doesn't apply to it the way it does to the other three.
**REGRESSION TEST:** None written — this is a static string swap with no branching logic to regress, verified instead by `npm run build` completing clean (2210 modules transformed, no errors) and a `grep -rn "gemini-3\.6\|gemma-4-26b" web/src/` sweep after the fix returning only the intentionally-unchanged fixture and one explanatory code comment.
...
**SEVERITY:** P1 — FIXED, 29 Aug 2026 (not yet deployed)
**AREA:** Frontend — v1's secondary navigation (`web/src/Shell.jsx`'s `Sidebar`) was completely inaccessible below Tailwind's `lg` breakpoint (1024px), with no fallback.
**FILE(S):** `web/src/Shell.jsx` (new `NAV_ITEMS` constant and `MobileNav` component), `web/src/App.jsx` (import + render).
**PROBLEM:** `Sidebar`'s root element was `className="hidden lg:flex ..."` with no narrower-viewport counterpart at all — not a hamburger menu, not a collapsed icon rail, nothing. View-switching (`view`/`setView`) is plain `useState` in `App.jsx`, not tied to routing or the URL, so there was no way — not even a manually-typed URL — to reach Pipeline, Autonomy Ledger, Evidence, Mission Theater, or Judge Mode below 1024px wide. Only the default "Command" view was ever reachable.
**HOW DISCOVERED:** Live browser inspection during the permission-driven full-check pass requested after two BUG-014 recurrences shook confidence in earlier "looks good" assurances. Confirmed structurally, not just visually, at both 959px and 767px viewport widths via direct JS: `getComputedStyle(sidebarEl).display === "none"` and `sidebarEl.getBoundingClientRect()` all zeros — i.e. the element has no size or presence in the layout at all, not merely hidden behind a toggle.
**IMPACT:** Real and significant for a hackathon demo specifically: judges very often view submissions on a phone, a tablet, or a non-maximized laptop window, and five of the six primary views (everything except the default Command screen) would have been completely unreachable to them with zero indication anything else existed.
**FIX:** Committed `23c69d4`. `Shell.jsx`'s previously-inline `items` array (used only by `Sidebar`) was pulled out to a module-level `NAV_ITEMS` constant. A new exported `MobileNav({ view, onView })` renders a `lg:hidden` horizontal, scrollable pill-button nav bar reading from that same constant, so `Sidebar` (desktop) and `MobileNav` (below `lg`) can never drift out of sync again. `App.jsx` imports `MobileNav` alongside the existing `Sidebar`/`Hero`/`TopStrip`/`CompletionRing` and renders `<MobileNav view={view} onView={setView} />` as the first child inside `<main>`, above `TopStrip`. Verified all six `NAV_ITEMS` keys (`command`/`pipeline`/`ledger`/`evidence`/`theater`/`judge`) match `App.jsx`'s `view === "..."` conditionals exactly.
**REGRESSION TEST:** None written (no existing frontend test harness for `web/` — `package.json` has only `dev`/`build`/`preview` scripts, no test runner). Verified instead by `npm run build` completing clean and a manual code-level check that `MobileNav`'s `onView` calls route through the identical `setView` used by `Sidebar`.
**VERIFICATION:** `npm run build` clean (2210 modules transformed, no errors), `vite.config.js`'s temporary `emptyOutDir: false` build workaround reverted with zero net diff afterward, confirmed via `git diff --stat`. Live visual confirmation in the browser at a narrow viewport is still pending deploy — the currently-deployed site still serves the pre-fix build (confirmed: navigating to the live `/v1` URL and checking `window.innerWidth` shows the fix isn't live yet).
**COMMIT:** `23c69d4` — "fix(v1): make Pipeline/Ledger/Evidence/Theater/Judge Mode reachable below 1024px".
**REMAINING WORK:** Needs a frontend-only deploy to go live: `git push` then `cd web && npm run build && npx firebase-tools deploy --only hosting`. No backend/`gcloud` redeploy needed — nothing in `app/` changed for this bug. Once deployed, worth a quick live check (narrow window or phone) that the pill nav appears and each item actually switches views.

---

## BUG-014

**SEVERITY:** P1 — FIXED, DEPLOYED, and LIVE-VERIFIED end-to-end, 29 Aug 2026
**AREA:** Capability invocation had no check that a mission step supplied the right number of arguments before running it. The real bug lived specifically in `app/synapse/engine.py`'s `_sandbox_proxy()` — the wrapper every SYNAPSE-installed (AI-generated) capability actually runs through. An earlier draft of this entry also flagged `app/workflows/orchestrator.py`'s `execute_tool()` as a likely site; that guess was correct in spirit but imprecise — see FIX for why a plain signature check there would NOT have caught this specific bug.
**FILE(S):** `app/synapse/engine.py` (`_sandbox_proxy()`, new `_entrypoint_arity()` static method — the load-bearing fix), `app/workflows/orchestrator.py` (`execute_tool()` — a secondary, narrower check, see FIX), `tests/test_synapse.py` (three new regression tests), `web/src/v5/AppV5.jsx` (UI hint, not enforcement).
**PROBLEM:** A real production mission (id truncated in the UI as `acdba94a`; full ID never needed — see below) failed with:
```
...
```
`generate_nepal_crisis_image` is a SYNAPSE-installed capability, which the registry stores not as its own real function but as `_sandbox_proxy()`'s `invoke(*args)` closure — a generic wrapper that builds a small harness script (`print(json.dumps({entrypoint}({call_args})))`) and runs it in the sandbox. Whatever mission step called it supplied zero arguments; `invoke()` had nothing checking that against what `generate_nepal_crisis_image` actually requires, so it built the harness anyway, the sandbox subprocess raised the real Python `TypeError`, and that raw stderr text became the mission's failure reason.
**HOW DISCOVERED:** Reported directly by the project owner, who ran a real mission against the live production deployment and saw it fail. Root cause was found by reading the execution code end-to-end rather than by obtaining the specific mission's full ID: tracing `app/missions/engine.py` → `app/workflows/orchestrator.py` → `execution_gate.execute()` → `tool.function(*args)` showed no validation anywhere — and then, critically, reading what `tool.function` actually *is* for an installed SYNAPSE capability (`_sandbox_proxy()`'s `invoke`, signature `(*args: Any)`) showed that a naive `inspect.signature(tool.function).bind(*args)` check at the orchestrator level would silently pass every call, real or broken, because the proxy itself accepts any number of arguments — the real requirement lives one layer deeper, in the entrypoint's own source inside `candidate["code"]`.
**IMPACT:** Any SYNAPSE-installed capability with required parameters could fail with a raw, user-facing Python traceback instead of a clean governance-style rejection, whenever a mission step under-supplied arguments — via either mission-execution path (`/missions/planned` and `/missions/from-graph`, i.e. the v5 graph builder). It also burned a real sandbox invocation on a call that could never have succeeded.
**FIX:** Two changes, committed `0c1c2ba`:
1. `app/synapse/engine.py`'s `_sandbox_proxy()`: before building the harness, `invoke()` now calls a new `_entrypoint_arity(code, entrypoint)` static method, which reads the entrypoint's required/allowed positional-argument count directly from its own source via Python's `ast` module — never executing the generated code, matching this file's existing "generated code never runs inside aion-core" rule (the same principle the safety screen already relies on). If the supplied `args` count is outside that range, `invoke()` returns `{"status": "ERROR", "error": "'<name>' needs N argument(s) but this call supplied M."}` immediately, without ever calling the sandbox. Functions using keyword-only parameters are left to the sandbox to report (the proxy's harness only ever calls positionally, so a keyword-only requirement can't be modeled as a positional range — and was already broken before this fix regardless).
2. `app/workflows/orchestrator.py`'s `execute_tool()`: added `inspect.signature(tool.function).bind(*args)` before `execution_gate.execute()`, mirroring the existing `CAPABILITY_GAP` check just above it. This is real but narrower than (1) — it only helps statically-registered capabilities (e.g. `calculator`, where `tool.function` is the genuine function), since a SYNAPSE-installed capability's `tool.function` is always the generic `_sandbox_proxy` closure this check can't see through. Kept because it's correct and free, not because it was the fix for the reported bug.
Also, `web/src/v5/AppV5.jsx`: the graph builder's node editor now fetches the selected capability's passport and shows its entrypoint's real required arguments next to the Args box (regex-read off the passport's own source, never fabricated — no hint shown if the passport has no code on file), so this mistake is visible before a mission is ever run, not just cleanly reported after.
**REGRESSION TEST:** Three new tests in `tests/test_synapse.py`: `test_installed_capability_rejects_too_few_arguments_before_the_sandbox_runs`, `test_installed_capability_rejects_too_many_arguments_before_the_sandbox_runs` (both assert the sandbox mock is never called), and `test_installed_capability_with_correct_argument_count_still_reaches_the_sandbox` (proves the fix doesn't block a legitimate call).
**VERIFICATION:** Full test suite run locally (`pytest tests/`, a fresh venv on the device bridge since the repo's own `.venv` is Windows-only): **615 passed, 4 skipped, 0 failed**, including the three new tests. `npm run build` (web/) completed clean with the frontend change included. A set of 4 failures in `test_mission_engine.py` (all about `write_brief` seed state) appears only when that file is run in isolation or paired with one other file, never in the full suite — confirmed pre-existing and unrelated by reproducing it identically with these changes stashed out via `git stash`.
**COMMIT:** `0c1c2ba` — "Fix BUG-014: catch a mission's wrong argument count before it runs". Pushed and fully deployed by the owner: `git push` (pre-push hook re-ran the suite: 615 passed, 4 skipped), frontend deploy (`firebase-tools deploy --only hosting`), and — after an initial false alarm where the exact same traceback recurred on a new mission (`7204ab5d`) because only the frontend had been deployed — the separate backend deploy (`gcloud run deploy aion-core --source . --region asia-south1 --project aion-axon-2026`, revision `aion-core-00048-lbq`, confirmed "serving 100 percent of traffic").
**REMAINING WORK:** None. Full end-to-end live verification completed 2026-08-29 with the owner unlocking a tab for direct testing (this session never read, entered, or handled the real owner token itself). Frontend half: selecting `generate_nepal_crisis_image` in the v5 graph builder triggers a real `GET .../capabilities/generate_nepal_crisis_image/passport` (200 OK) and correctly displays "generate_nepal_crisis_image needs: input_str: str". Backend half: the v5 graph builder's own node editor cannot actually construct a true zero-argument call (a new node always seeds `args: [""]`, and the Args textarea's `split("\n")` of an empty string still yields `[""]`, never `[]`) — confirmed by intercepting the real `POST /missions/from-graph` request body via a page-level `fetch` patch (editing only the JSON body, never the request's auth headers). A `[""]` (one empty-string argument) call correctly succeeded (arity of 1 satisfied 1 required), producing a real image. The same interception was then used to force the request body's `args` to a true `[]`, reproducing the original production bug exactly, and the live response came back as `MISSION 13BDC15C — FAILED — 'generate_nepal_crisis_image' needs 1 argument but this call supplied 0.` — the new clean, governed message, not a raw Python traceback. This also revealed that the real production failures (`acdba94a`, `7204ab5d`) could only have come from the natural-language planner path (`/missions/planned`), where `MissionStep.args` is LLM-generated and can genuinely be `[]` when the planner doesn't know a tool's signature (see `app/missions/service.py`'s own `resume_blocked()` docstring, which documents an earlier, already-fixed instance of this same zero-arg failure mode for the capability-gap-resume path specifically) — not the graphical builder, which structurally cannot produce zero arguments.
