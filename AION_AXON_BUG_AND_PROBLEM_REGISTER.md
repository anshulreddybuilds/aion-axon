...
**SEVERITY:** P1 — FIXED and DEPLOYED, 29 Aug 2026 (backend live-verification of the exact failure case still pending, see REMAINING WORK)
...
**REMAINING WORK:** Frontend half confirmed live via browser automation, 2026-08-29: selecting `generate_nepal_crisis_image` in the v5 graph builder triggers a real `GET .../capabilities/generate_nepal_crisis_image/passport` (200 OK) and correctly displays "generate_nepal_crisis_image needs: input_str: str". The backend half (submitting an actual mission with an empty-args node and confirming it now ends `ERROR` with the new sentence instead of a raw traceback) still needs a live end-to-end check with the real owner token — that must be done by the owner in his own browser, or with him unlocking a tab for direct testing; it is not something this session enters or uses itself. Not required for confidence (the 3 new unit tests already exercise `_entrypoint_arity` directly against the sandbox-call boundary), but would be the final close-out step on this bug.
